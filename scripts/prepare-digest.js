...
async function main() {
  ...
  // 3. Load prompts with priority:
  //    user custom > bundled Chinese defaults (zh mode) > remote > local fallback
  //
  ...
  for (const filename of PROMPT_FILES) {
    ...
    // Chinese mode is localized by this skill. Keep these prompts authoritative
    // instead of replacing them with the centrally hosted English templates.
    if (config.language === 'zh' && existsSync(localPath)) {
  ...
  const output = {
    ...
    config: {
      ...
      frequency: config.frequency || 'on-demand',
